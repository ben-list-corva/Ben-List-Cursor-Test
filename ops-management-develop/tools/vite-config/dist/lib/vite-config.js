...
import { CORVA_API_ENVS, CORVA_API_ENV_VARIABLES, CORVA_GLOBAL_DEPENDENCIES,
...
export const createViteConfig = ({ name, pwd }) => {
    const __dirname = pwd;
    expand(dotenv.config());
    const packageJson = JSON.parse(readFileSync(resolve(__dirname, './package.json'), 'utf-8'));
    const rootPackageJson = JSON.parse(readFileSync(resolve(__dirname, '../../package.json'), 'utf-8'));
    const manifestJson = JSON.parse(readFileSync(resolve(__dirname, './manifest.json'), 'utf-8'));
    const { application: { key: appKey }, } = manifestJson;
    const getAppIdentifier = (version = packageJson.version) => {
        const appIdentifier = appKey.replace(/\./g, '_');
        return `${appIdentifier}-${version}`;
    };
    const getAlias = async () => {
        const tsconfig = JSON.parse(await readFile(join(__dirname, 'tsconfig.app.json'), 'utf-8'));
        if (!tsconfig.compilerOptions?.paths) {
            return {};
        }
        const alias = Object.fromEntries(Object.entries(tsconfig.compilerOptions.paths).map(([key, value]) => [
            key.replace('/*', ''),
    ...
    const getCorvaApiEnvVariables = () => {
        ...
        if (apiEnv in CORVA_API_ENVS)
            return CORVA_API_ENV_VARIABLES[apiEnv];
        console.log(chalk.red(`CORVA_API_ENV should be one of [${Object.keys(CORVA_API_ENVS).join(', ')}]`));
        return process.exit(1);
    };
    const CORVA_EXTERNALS = Object.keys(CORVA_GLOBAL_DEPENDENCIES).map((libName) => [
        libName,
    ...
    Object.keys(CORVA_GLOBAL_DEPENDENCIES).forEach((libName) => {
        if (libName.startsWith('react/jsx-runtime') ||
            libName.startsWith('highcharts/')) {
            return;
        }
        // Convert @scope/package-name to valid JS identifier
        staticGlobals[libName] = `this["${libName}"].default || this["${libName}"]`;
    });
    ...
    return defineConfig(async (args) => {
                ...
                rollupOptions: {
                    ...
                    external: (source, importer, isResolved) => {
                        for (let i = 0; i < SKIP_IN_EXTERNALS.length; i += 1) {
                            if (source.startsWith(SKIP_IN_EXTERNALS[i]))
                                return false;
                        }
        ...
        if (args.command === 'build') {
            config.build = {
                ...
                rollupOptions: {
                    ...
                    plugins: [
                        ...
                        license({
                            thirdParty: {
                                output: join(config.build?.outDir || `../../dist/apps/${name}`, 'build/app.js.LICENSE.txt'),
                            },
                        }),
            ...
            config.optimizeDeps = {
                ...
                exclude: [
                    ...
                    ...Object.keys(CORVA_GLOBAL_DEPENDENCIES),
                ],
            };
                ...
                targets: [
                    ...
                    },
                    {
                        ...
                        transform: (content, filename) => {
                            ...
                            const highchartsVersion = rootPackageJson?.dependencies?.highcharts ||
            ...
            const fallbackForEnv = Object.entries(process.env).reduce((acc, [key, value]) => {
            ...
            config.plugins?.push(DynamicPublicDirectory([
                '../../public/**',
                {
